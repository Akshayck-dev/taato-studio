...
export default function Page() {
  ...
  return (
    <BookingProvider>
      {currentPage !== 'booking' && currentPage !== 'admin' && (
        <Navigation 
      ...
      <main className="bg-background">
        ...
        {currentPage === 'services' && <div className="py-8"><Services onBookNow={() => setCurrentPage('booking')} /></div>}
        {currentPage === 'artists' && <div className="py-8"><Artists onBookNow={() => setCurrentPage('booking')} /></div>}
        {currentPage === 'portfolio' && <div className="py-8"><Portfolio onBookNow={() => setCurrentPage('booking')} /></div>}
        {currentPage === 'testimonials' && <div className="py-8"><Testimonials /></div>}
        {currentPage === 'pricing' && <div className="py-8"><Pricing onBookNow={() => setCurrentPage('booking')} /></div>}
        {currentPage === 'aftercare' && <div className="py-8"><AftercarePage /></div>}
        {currentPage === 'faq' && <div className="py-8"><FAQ /></div>}
        {currentPage === 'contact' && <div className="py-8"><Contact /></div>}
        {currentPage === 'booking' && <BookingPage onClose={() => setCurrentPage('home')} />}
        {currentPage === 'admin' && <AdminPanel onLogout={() => setCurrentPage('home')} />}
      ...
      {!['booking', 'admin', 'user-dashboard', 'admin-dashboard', 'artist-dashboard'].includes(currentPage) && (
        <Footer />
      )}
